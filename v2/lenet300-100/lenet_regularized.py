...
class TrainingHistory(Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		self.weights.append(self.model.layers[2].get_weights()[0])
...
if args.l1 == 'normal':	
	model.add(Dense(300, input_dim=input_dim, bias=False, weights=[W]))
else:
	coeff = np.polynomial.polynomial.polyfit(np.arange(input_dim) + 1.0, W, deg=args.deg1)
	model.add(PolyDense(300, input_dim=input_dim, deg = args.deg1, weights=[coeff], W_regularizer = reg)) 

model.add(Activation(args.activ)) 

W = normal((model.output_shape[-1], 100)).eval()
if args.l2 == 'normal':
	model.add(Dense(100, bias=False, weights=[W], trainable=False))
else:
...
model.fit(X_train, Y_train,
					batch_size=batch_size, nb_epoch=args.epoch, callbacks=[earlystopping, history],
